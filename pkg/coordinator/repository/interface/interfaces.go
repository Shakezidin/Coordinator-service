...
type CoordinatorRepoInter interface {
	...
	FetchActivity(id uint) (*cDOM.Activity, error)
	UpdatePassword(id uint, newpassword string) error
	CreateCategory(catagory cDOM.Category) error
	FetchAllPackages(offset, limit int) ([]*cDOM.Package, error)
	PackageStatusUpdate(id uint) error
	FetchCategories(offset, limit int) ([]*cDOM.Category, error)
	...
	FetchCategory(catagory string) (*cDOM.Category, error)
